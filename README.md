...
- 「PDF 內容擷取」可輸出可編輯 Word（`.docx`）、表格 Excel（`.xlsx`）或純文字（`.txt`）。
- Word 依來源檔案與頁面建立可編輯段落及分頁，不承諾完全重現原始版面；TXT 只保留文字、檔名與頁碼。
- Excel 依文字座標推測列與欄，每份來源 PDF 建立獨立工作表；複雜格線、合併儲存格與無框線表格可能無法正確還原。
- 三種內容擷取格式都可選擇使用 Tesseract.js OCR 處理沒有文字層的掃描頁面；首次使用需下載並由瀏覽器快取語言資料。
